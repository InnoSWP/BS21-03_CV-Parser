...
Future<void> main() async {
  ...
  group('Input text tests',(){
    ...
    test("The resume must contain only the English alphabet and mathematical symbols",(){
     for(int i = 0; i < cooler_list[0].coolText.length;i++ ){
      expect(cooler_list[0].coolText.codeUnitAt(i) < 127, true);
     }
    });
  });
  ...
  for(int i = 0; i < length; i++){
    ...
    if(cooler_list[0].cvString[i] == String.fromCharCode(92) && cooler_list[0].cvString[i + 1] == 'n'){
      if(DEBUG){
        print(counter);
      } 
      counter = false;
    ...
    if(counter){
      if(cooler_list[0].cvString[i] == ' '){
        for(int j = i; j < cooler_list[0].cvString.length ; j++){
          if(cooler_list[0].cvString[j] == ',' || cooler_list[0].cvString[j] == String.fromCharCode(92)){
            if(DEBUG){
              bool search_string = cooler_list[0].searchFor(cooler_list[0].cvString.substring(i, j)); 
            // expect(true,true);
              print(cooler_list[0].cvString.substring(i, j));
              print(search_string);
            }
            test('Search word should exist in file', (){
              expect(cooler_list[0].searchFor(cooler_list[0].cvString.substring(i, j)), true);
            });
            i = j;
            break;
          }
